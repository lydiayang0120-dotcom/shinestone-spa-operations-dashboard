...
function fixtures() {
  ...
  const names=['中壢岩盤浴','八德岩盤浴','台中岩盤浴','台北岩盤浴','忠孝岩盤浴','板橋岩盤浴','桃園岩盤浴','羅東岩盤浴','青埔岩盤浴'];
  for(let m=0;m<12;m++) {
    for(const name of names) {
      newRows.push({'月份':data.months[m],'品牌代碼':'SS','年度歸屬':2026,'年度月序':m+1,'分店':name,'資料狀態':m<2?'已匯入':'待匯入','實際新客':999,'到店數':name==='中壢岩盤浴'?2:name==='八德岩盤浴'?4:0,'新客目標':10});
      if(m<2) metaRows.push({'月份':data.months[m],'品牌代碼':'SS','年度歸屬':2026,'月序':m+1,'分店':name,'實際花費':100,'訊息花費':80,'Meta詢問數':4,'Meta預算':120,'資料完整性':'已確認','查詢備註':''});
    }
    if(m<2) metaRows.push({'月份':data.months[m],'品牌代碼':'SS','年度歸屬':2026,'月序':m+1,'分店':'品牌整體','實際花費':0,'訊息花費':0,'Meta詢問數':0,'Meta預算':0,'資料完整性':'已確認','查詢備註':'無法歸屬單店的品牌活動'});
    for(const platform of ['Meta','G關鍵字','G多媒體','LAP']) budgetRows.push({'月份':data.months[m],'品牌代碼':'SS','年度歸屬':2026,'年度月序':m+1,'廣告平台':platform,'預算金額':300,'實際花費':m<2?(platform==='Meta'?900:platform==='G關鍵字'?20:platform==='G多媒體'?30:10):null,'實際花費來源':'測試資料','填寫狀態':m<2?'已填':'待補'});
  }
  const overview=[],detail=[];
  for(let m=0;m<2;m++) {
    overview.push({'月份':data.months[m],'品牌代碼':'SS','客群':'新客','到店數':6,'消費數':3,'消費金額':700});
    overview.push({'月份':data.months[m],'品牌代碼':'SS','客群':'舊客','到店數':m,'消費數':m,'消費金額':m*50});
    for(const name of names) { const arrivals=name==='中壢岩盤浴'?2:name==='八德岩盤浴'?4:0,count=name==='中壢岩盤浴'?1:name==='八德岩盤浴'?2:0,amount=name==='中壢岩盤浴'?100:name==='八德岩盤浴'?600:0; detail.push({'月份':data.months[m],'品牌代碼':'SS','客群':'新客','統計基準':'依預約日期','預約店':name,'來源店名':name,'來源代碼':1,'到店數':arrivals,'消費數':count,'消費金額':amount}); }
    if(m) detail.push({'月份':data.months[m],'品牌代碼':'SS','客群':'舊客','統計基準':'依預約日期','預約店':'八德岩盤浴','來源店名':'八德岩盤浴','來源代碼':1,'到店數':1,'消費數':1,'消費金額':50});
  }
...
test('fiscal year, confirmed months and message-cost CPA',()=>{
  ...
  assert.deepEqual(data.storeCodes,['zl','bd','tc','tp','zx','bc','ty','ld','qp']);
  assert.equal(r.newRows[0].zl.a,2);
...
test('missing Meta zero placeholders remain unknown; confirmed zero stays zero',()=>{
  ...
  assert.equal(data.metaTotal([r[0]],'zl').s,null);
  assert.equal(data.metaTotal([r[0]],'all').status,'部分資料');
  Object.assign(raw[1][0],{'資料完整性':'已確認','查詢備註':''});
  const zero=data.metaTotal([data.parseMetaRows(table(raw[1]))[0]],'zl');
...
test('duplicate keys, missing new-customer months and invalid numbers fail closed',()=>{
  ...
  assert.throws(()=>data.parseNewRows(table(raw[0].filter(r=>r['年度月序']!==1))),/連續|完整/);
...
test('partial-platform budgets are not reported complete',()=>{
  ...
  assert.equal(data.budgetAggregate(r[0],'all').a,950);
...
test('UI login, refresh, four panels, consumption filters and logout',async()=>{
  ...
  elements.get('#sms-consumption-mode').value='monthly';elements.get('#sms-consumption-store').value='bd';elements.get('#sms-consumption-cohort').value='returning';
  ...
  elements.get('#sms-new-mode').value='monthly';elements.get('#sms-new-store').value='zl';
...
test('consumption is reconciled by month, cohort and store; weighted average uses arrivals',()=>{
  ...
  assert.equal(data.consumptionTotal(rows,'zl','new').amount,200);
  ...
  assert.equal(data.consumptionTotal(rows,'zl','returning').amount,null);
...
test('cross-period nine-store union keeps monthly unlisted stores null',()=>{
  const names=['中壢岩盤浴','八德岩盤浴','台中岩盤浴','台北岩盤浴','忠孝岩盤浴','板橋岩盤浴','桃園岩盤浴','羅東岩盤浴','青埔岩盤浴'];
  const missing=[
    ['八德岩盤浴','青埔岩盤浴'],
    ['台北岩盤浴','八德岩盤浴','青埔岩盤浴'],
  ...
  const monthly=[],overview=[],detail=[];
  for(let m=0;m<8;m++) {
    const active=names.filter(name=>!missing[m].includes(name));
    for(const name of active) {
      monthly.push({'月份':data.months[m],'品牌代碼':'SS','年度歸屬':2026,'年度月序':m+1,'分店':name,'資料狀態':'已匯入','實際新客':999,'到店數':1,'新客目標':null});
      detail.push({'月份':data.months[m],'品牌代碼':'SS','客群':'新客','統計基準':'依預約日期','預約店':name,'來源店名':name,'來源代碼':1,'到店數':1,'消費數':1,'消費金額':100});
    }
    overview.push({'月份':data.months[m],'品牌代碼':'SS','客群':'新客','到店數':active.length,'消費數':active.length,'消費金額':active.length*100});
    overview.push({'月份':data.months[m],'品牌代碼':'SS','客群':'舊客','到店數':0,'消費數':0,'消費金額':0});
  }
  const newRows=data.parseNewRows(table(monthly));
  assert.equal(newRows.length,8);
  assert.deepEqual(newRows[0].bd,{a:null,t:null,active:false});
  assert.deepEqual(newRows[0].tp,{a:1,t:null,active:true});
  assert.deepEqual(newRows[1].tp,{a:null,t:null,active:false});
  assert.deepEqual(newRows[4].bd,{a:1,t:null,active:true});
  assert.deepEqual(newRows[4].qp,{a:1,t:null,active:true});
  assert.equal(data.newTotal([newRows[0]],'all').a,7);
  assert.equal(data.newTotal([newRows[1]],'all').a,6);
  assert.equal(data.newTotal([newRows[4]],'all').a,8);
  assert.equal(data.newTotal(newRows,'tp').a,1);
  const consumption=data.parseConsumptionRows(table(overview),table(detail));
  assert.equal(consumption[0].new.bd.present,false);
  assert.equal(consumption[0].new.tp.present,true);
  assert.equal(consumption[1].new.tp.present,false);
  assert.equal(consumption[4].new.bd.present,true);
  assert.equal(consumption[4].new.qp.present,true);
  assert.equal(data.consumptionTotal([consumption[0]],'bd','new').amount,null);
  assert.equal(data.consumptionTotal(consumption,'tp','new').amount,100);
  assert.equal(data.consumptionTotal([consumption[0]],'all','new').amount,700);
});
test('unknown store names are rejected instead of being merged into SS',()=>{
  const monthly=fixtures();
  monthly[0][0]['分店']='未設定岩盤浴';
  assert.throws(()=>data.parseNewRows(table(monthly[0])),/未設定/);
  const consumption=fixtures();
  consumption[4][0]['預約店']='舊名稱岩盤浴';
  consumption[4][0]['來源店名']='舊名稱岩盤浴';
...
test('missing targets keep the aggregate incomplete; confirmed zero remains zero',()=>{
  ...
  rows[0].qp.t=0;
  assert.deepEqual(data.newTotal(rows,'all'),{a:45,t:360,d:-315,rate:45/360});
  const raw=fixtures();raw[0][0]['新客目標']=null;
  assert.equal(data.parseAll(ranges(raw)).newRows[0].zl.t,null);
