(function (host) {
  ...
  const isBrand = row => row['品牌代碼'] === 'SS';
  const selected = row => isBrand(row) && Number(row['年度歸屬']) === 2026;
  const storeCodes = ['zl','bd','tc','tp','zx','bc','ty','ld','qp'];
  ...
  function parseNewRows(values) {
    ...
    for (const row of result) for (const code of storeCodes) if (!row[code]) row[code] = {a:null,t:null,active:false};
    return result;
  }
  ...
  const emptyConsumption = present => ({amount:0,count:0,arrivals:0,present});
  ...
  function parseConsumptionRows(overview, detail) {
    ...
    for (const row of records(overview, ['月份','品牌代碼','客群','到店數','消費數','消費金額'])) {
      ...
      add(grouped, m, cohort, {total:consumptionValues(row),...Object.fromEntries(storeCodes.map(code=>[code,emptyConsumption(false)]))});
    }
    for (const row of records(detail, ['月份','品牌代碼','客群','統計基準','預約店','來源代碼','到店數','消費數','消費金額','來源店名'])) {
      ...
      if (!group[code].present) group[code] = emptyConsumption(true);
    ...
    for (const row of result) for (const cohort of ['new','returning']) {
      ...
      for (const field of consumptionFields) {
        if (Math.abs(storeCodes.reduce((total,code)=>total+(group[code].present?group[code][field]:0),0) - group.total[field]) > 0.01) throw new Error(`${months[row.m]} 消費明細與月報總覽不一致，停止顯示。`);
      }
    }
    return result;
  }
  function consumptionTotal(rows, store = 'all', cohort = 'new') {
    ...
    const result = {amount:0,count:0,arrivals:0};
    let present = false;
    ...
    if (!present) return {amount:null,count:null,arrivals:null,average:null,rate:null};
  ...
  function parseAll(ranges) {
    ...
    for(const row of newRows) for(const code of storeCodes) {
      const consumption=consumptionRows.find(r=>r.m===row.m)?.new[code];
      const actual=consumption?.present?consumption.arrivals:null;
      if(row[code].active!==Boolean(consumption?.present) || row[code].a!==actual) throw new Error('新客到店實績與分店消費到店數不一致。');
    }
