(() => {
  ...
  function updateNew() {
    ...
    const {a,t,d,rate}=newTotal(active,store),hasData=a!==null;
    $('#sms-new-actual').innerHTML=hasData?unit(a,'人'):'—';
    $('#sms-new-target').innerHTML=hasData?unit(t,'人'):'—';
    $('#sms-new-rate').textContent=!hasData?'—':t===null?'待補目標':pct(rate);
    $('#sms-new-gap').innerHTML=!hasData?'—':d===null?'待補目標':unit(Math.abs(d),'人');
    $('#sms-new-gap-note').textContent=!hasData?'所選期間月報未列此分店':d===null?'目標確認後自動計算':storeName(store)+(d>=0?'超前':'落後')+' '+fmt(Math.abs(d));
    $('#sms-new-period').textContent=mode==='monthly'?months[cutoff]:'2025/12－'+months[cutoff];
    $('#sms-new-target-note').textContent=!hasData?'無可計算的分店實績':t===null?'所選期間尚有未提供的目標':mode==='monthly'?'單月目標':'已計入 '+rows.length+' 個月份';
    ...
    $('#sms-store-progress').innerHTML=storeCodes.map(code=>{
      ...
      if(v.a===null) return '<div class="sms-store"><div class="sms-store-title"><strong>'+storeName(code)+'</strong><span>無資料</span></div><div class="sms-store-meta"><span>截至所選月份尚未列入月報</span></div></div>';
    ...
    const cells=selection=>{
      ...
      return storeCodes.map(code=>{
        const v=newTotal(selection,code);
        return v.a===null?'<td tabindex="0" title="該月報未列此分店">—</td>':'<td tabindex="0" title="實績 '+fmt(v.a)+' / 目標 '+fmt(v.t)+'">'+fmt(v.a)+' / '+fmt(v.t)+'</td>';
      }).join('')+'<td>'+fmt(total.a)+' / '+fmt(total.t)+'</td><td>'+(filtered.a===null?'—':filtered.t===null?'待補目標':pct(filtered.rate))+'</td>';
    };
  ...
  function updateConsumption() {
    ...
    const cohortName=cohort==='new'?'新客':cohort==='returning'?'舊客':'新客＋舊客',hasData=total.amount!==null;
    $('#sms-consumption-amount').innerHTML=hasData?unit(total.amount,'元'):'—';
    $('#sms-consumption-count').innerHTML=hasData?unit(total.count,'筆'):'—';
    $('#sms-consumption-average').innerHTML=total.average===null?'—':unit(total.average,'元');
    $('#sms-consumption-rate').textContent=pct(total.rate);
    $('#sms-consumption-arrivals').textContent=total.arrivals===null?'所選期間月報未列此分店':'消費數 ÷ 到店數｜到店 '+fmt(total.arrivals)+' 筆';
    $('#sms-consumption-period').textContent=(mode==='monthly'?months[cutoff]:'2025/12－'+months[cutoff])+'｜'+storeName(store)+'｜'+cohortName;
    const knownAmounts=rows.map(r=>consumptionTotal([r],store,cohort).amount).filter(Number.isFinite);
    const max=Math.max(...knownAmounts,1);
    $('#sms-consumption-bars').innerHTML=rows.map(r=>{
      const v=consumptionTotal([r],store,cohort),known=v.amount!==null;
      return '<div class="sms-bar-row"><span>'+months[r.m].slice(2)+'</span><div class="sms-bar-track"><div class="sms-bar" style="width:'+(known?v.amount/max*100:0)+'%;min-width:0"></div></div><span class="sms-bar-value" title="'+months[r.m]+' '+(known?fmt(v.amount)+' 元':'無資料')+'">'+(known?fmt(v.amount):'—')+'</span></div>';
    }).join('');
    const combined=consumptionTotal(active,'all',cohort).amount;
    $('#sms-consumption-stores').innerHTML=storeCodes.map((code,index)=>{
      const v=consumptionTotal(active,code,cohort);
      if(v.amount===null) return '<div class="sms-store"><div class="sms-store-title"><strong>'+storeName(code)+'</strong><span>無資料</span></div><div class="sms-store-meta"><span>所選期間月報未列此分店</span></div></div>';
      const share=combined>0?v.amount/combined:null;
      return '<div class="sms-store"><div class="sms-store-title"><strong>'+storeName(code)+'</strong><span>'+pct(share)+'</span></div><div class="sms-track"><div class="sms-fill '+(index%2?'sage':'')+'" style="width:'+((share||0)*100)+'%"></div></div><div class="sms-store-meta"><span>'+fmt(v.amount)+' 元</span><span>消費 '+fmt(v.count)+' 筆</span></div></div>';
    }).join('');
    const cells=v=>v.amount===null?'<td>—</td><td>—</td><td>—</td><td>—</td><td>—</td>':'<td>'+fmt(v.amount)+'</td><td>'+fmt(v.count)+'</td><td>'+fmt(v.arrivals)+'</td><td>'+(v.average===null?'—':fmt(v.average))+'</td><td>'+pct(v.rate)+'</td>';
